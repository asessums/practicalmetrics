...
This is an R Markdown document. Markdown is a simple, reproducible tool for authoring HTML, PDF, and MS Word documents. For more details on using R Markdown as an Analytics tool, you can reference the RStudio website at <http://rmarkdown.rstudio.com>.
...
Last week, the firm submitted `r nrow(proposals)` proposals submitted worth `r format(sum(proposals$Fees), scientific = FALSE)`, or `r format(sum(proposals$Fees) / nrow(proposals), scientific = FALSE)` per proposal. But, roses are \textcolor{red}{red}, violets are \textcolor{blue}{blue}
